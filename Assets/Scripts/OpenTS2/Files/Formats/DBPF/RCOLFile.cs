...
namespace OpenTS2.Files.Formats.RCOL
{
    // from https://github.com/LazyDuchess/SimUnity2/blob/master/Assets/Scripts/Files/RCOLFile.cs
            ...
            for (var i = 0; i < loopCount; i++)
            {
                ...
                for (var j = 0; j < innerLoopCount; j++)
                {
                    ...
                    else
                    {
                        ...
                        var lifoAsset = ContentManager.Get().Provider.GetFromResourceMap(new ResourceKey(lifoName, 0x1C0532FA, TypeIDs.LIFO));
                        if (lifoAsset == null)
                            return thisData;
    ...
    public class ModelWrapper
    {
        ...
        {
            var modelGameObject = new GameObject(Name);
            Debug.Log($@"
groupMaterialDefs:
    Length: {groupMaterialDefs.Count}
...
");
            for (int i = 0; i < GMDCData.model.meshes.Count; i++)
            {
                ...
                var renderer = thisGroup.AddComponent<MeshRenderer>();
                Debug.Log("materialList length: " + filter.sharedMesh.subMeshCount);
                var materialList = new Material[filter.sharedMesh.subMeshCount];
                var referenceList = new List<CacheKey>();
                for (int j = 0; j < materialList.Length; j++)
                {
                    var nMat = new Material(Shader.Find("Specular"));
                    nMat.name = GMDCData.Groups[i].groupName;
                    if (groupMaterialDefs.ContainsKey(nMat.name))
                    {
                        var matDef = groupMaterialDefs[nMat.name];
    ...
    public class RCOLFile
    {
        ...
        public List<ResourceKey> references = new List<ResourceKey>();
        ...
        public RCOLFile(DBPFEntry reference, bool imposterTest = false)
        {
            imposter = imposterTest;
            var stream = new MemoryStream(reference.GetBytes());
            ...
            for (var i = 0; i < fileLinks; i++)
            {
                ...
                if (groupID == 0xFFFFFFFF)
                    groupID = reference.GlobalTGI.GroupID;
                var instanceID = reader.ReadUInt32();
                uint resourceID = 0x0;
                if (referenceType == ReferenceType.TGIR)
                    resourceID = reader.ReadUInt32();
                var typeID = reader.ReadUInt32();
                var entryID = new ResourceKey(instanceID, groupID, typeID);
                if (referenceType == ReferenceType.TGIR)
                    entryID = new ResourceKey(instanceID, resourceID, groupID, typeID);
        ...
        public static ModelWrapper GetModel(string name)
        {
            ...
            foreach (var element in testCres.dataBlocks)
            {
                if (element.BlockName == "cShapeRefNode")
                {
                    ...
                    {
                        DBPFEntry shape = ContentManager.Get().Provider.GetFromResourceMap(testCres.references[(int)shapeLink]);
                        if (shape != null)
                        {
